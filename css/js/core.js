"use strict";
class Dialog {
    promptPromise = { resolve: null, reject: null };
    dialog = null;
    elements = [];
    _nextElementId = 0;
    ...
    set title(title) {
        ...
        if(dialogTitle) {
            ...
            else dialogTitle.remove();
        } else {
            if(title) {
                dialogTitle = document.createElement("div");
                dialogTitle.classList.add("dialog-title");
                if(dialogContent.firstChild) dialogContent.insertBefore(dialogTitle, dialogContent.firstChild);
                else dialogContent.appendChild(dialogTitle);
    ...
    get title(){
        const dialogContent = this.dialog.querySelector(".dialog-content");
        let dialogTitle = dialogContent.querySelector(".dialog-title");
        return dialogTitle ? dialogTitle.textContent : null;
    }
    set text(text) {
        const dialogContent = this.dialog.querySelector(".dialog-content");
        let dialogText = dialogContent.querySelector(".dialog-text");
        if(dialogText) {
            if(text) dialogText.textContent = text;
            else dialogText.remove();
        } else {
            if(text) {
                dialogText = document.createElement("div");
                dialogText.classList.add("dialog-text");
                if(dialogContent.querySelector("button")) dialogContent.insertBefore(dialogText, dialogContent.querySelector("button"));
                else dialogContent.appendChild(dialogText);
    ...
    handleBlur(event){ this.focusFirstElement(); }
    focusFirstElement(){
...
class DialogElement {
    ...
    add(dialog) { dialog.addElement(this); return this; }
    getElement(){ return; }
    onRender(){ return; }
}
class DialogButton extends DialogElement {
    ...
    set type(type) {
        ...
        types.forEach(t => this.button.classList.remove("dialog-"+t));
        this.button.classList.add("dialog-"+type);
    }
    get type() {
        const types = ["normal", "positive", "negative"];
        for(let t of types)
            if(this.button.classList.contains("dialog-"+t)) return t;
        return null;
    }
    setEnabled(enabled) { this.enabled = enabled; return this; }
    set enabled(enabled) { this.button.disabled = !enabled; }
    get enabled(){ return !this.button.disabled; }
    setBold(bold) { this.bold = bold; return this; }
    set bold(bold) {
        if(bold) this.button.classList.add("bold");
        else this.button.classList.remove("bold");
    }
    get bold(){ return this.button.classList.contains("bold"); }
    setText(text) { this.text = text; return this; }
    set text(text) { this.button.textContent = text; }
    get text(){ return this.button.textContent; }
    remove() { this.button.remove(); }
    _createButton(){ this.button = document.createElement("button"); }
    getElement(){ return this.getButton(); }
}
class DialogInput extends DialogElement {
    ...
    setType(type) { this.type = type; return this; }
    set type(type) { this.input.type = type; }
    get type() { return this.input.type; }
    setText(text) { this.text = text; return this; }
    set text(text) { this.input.value = text; }
    get text() { return this.input.value; }
    set value(value){ this.text = value }
    get value(){ return this.text; }
    setPlaceholder(placeholder) { this.placeholder = placeholder; return this; }
    set placeholder(placeholder) { this.input.placeholder = placeholder; }
    get placeholder() { return this.input.placeholder; }
    setEnabled(enabled) { this.enabled = enabled; }
    set enabled(enabled) { this.input.disabled = !enabled; }
    get enabled() { return !this.input.disabled; }
    remove() { this.input.remove(); }
    _createInput(){ this.input = document.createElement("input"); }
    getElement(){ return this.getInput(); }
}
class DialogTextarea extends DialogElement {
    ...
    getTextarea() { return this.textarea; }
    setText(text) { this.text = text; return this; }
    set text(text) { this.textarea.value = text; }
    get text() { return this.textarea.value; }
    set value(value){ this.text = value }
    get value(){ return this.text; }
    setPlaceholder(placeholder) { this.placeholder = placeholder; return this; }
    set placeholder(placeholder) { this.textarea.placeholder = placeholder; }
    get placeholder() { return this.textarea.placeholder; }
    setEnabled(enabled) { this.enabled = enabled; }
    set enabled(enabled) { this.textarea.disabled = !enabled; }
    get enabled() { return !this.textarea.disabled; }
    remove() { this.textarea.remove(); }
    _createTextarea(){
    ...
    getElement(){ return this.getTextarea(); }
    onRender(){ this.handleInput(); }
}
